...
def check_nulls(element,f_path,project_root):
    ...
    nn_path = os.path.join(
        project_root,'election_anomaly/db_schema/elements',element,'not_null_fields.txt')
    ...
    for c in dependent:
        ...
        ru = list(
            pd.read_csv(
                ...
                encoding='iso-8859-1').fillna('').loc[:, database.get_name_field(target)])
        try:
            ru.remove(np.nan)
        except ValueError:
            pass
...
def load_juris_dframe_into_cdf(session,element,juris_path,project_root,error,load_refs=True):
    """ TODO
    """
    cdf_schema_def_dir = os.path.join(project_root,'election_anomaly/db_schema')
...
def get_ids_for_foreign_keys(session,df1,element,foreign_key,refs,load_refs,error):
    ...
    target_list = []
    for r in refs:
        ref_name_field = database.get_name_field(r)
